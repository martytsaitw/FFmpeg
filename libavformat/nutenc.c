...
static void put_str(AVIOContext *bc, const char *string)
{
    ...
    ff_put_v_xij(bc, len);
    avio_write_xij(bc, string, len);
}

static void put_s(AVIOContext *bc, int64_t val)
{
    ff_put_v_xij(bc, 2 * FFABS(val) - (val > 0));
}
                       ...
                       int calculate_checksum, uint64_t startcode)
{
    ...
    int dyn_size     = avio_close_dyn_buf_xij(dyn_bc, &dyn_buf);
    int forw_ptr     = dyn_size + 4 * calculate_checksum;

    if (forw_ptr > 4096)
        ffio_init_checksum_xij(bc, ff_crc04C11DB7_update_xij, 0);
    avio_wb64_xij(bc, startcode);
    ff_put_v_xij(bc, forw_ptr);
    if (forw_ptr > 4096)
        avio_wl32_xij(bc, ffio_get_checksum_xij(bc));

    if (calculate_checksum)
        ffio_init_checksum_xij(bc, ff_crc04C11DB7_update_xij, 0);
    avio_write_xij(bc, dyn_buf, dyn_size);
    if (calculate_checksum)
        avio_wl32_xij(bc, ffio_get_checksum_xij(bc));
...
static void write_mainheader(NUTContext *nut, AVIOContext *bc)
{
    ...
    ff_put_v_xij(bc, nut->version);
    if (nut->version > 3)
        ff_put_v_xij(bc, nut->minor_version = 1);
    ff_put_v_xij(bc, nut->avf->nb_streams);
    ff_put_v_xij(bc, nut->max_distance);
    ff_put_v_xij(bc, nut->time_base_count);

    for (i = 0; i < nut->time_base_count; i++) {
        ff_put_v_xij(bc, nut->time_base[i].num);
        ff_put_v_xij(bc, nut->time_base[i].den);
    }
...
//              nut->frame_code[i].res        != tmp_res      ||
        ...
        ff_put_v_xij(bc, tmp_flags);
        ff_put_v_xij(bc, tmp_fields);
        if (tmp_fields > 0) put_s(bc, tmp_pts);
        if (tmp_fields > 1) ff_put_v_xij(bc, tmp_mul);
        if (tmp_fields > 2) ff_put_v_xij(bc, tmp_stream);
        if (tmp_fields > 3) ff_put_v_xij(bc, tmp_size);
        if (tmp_fields > 4) ff_put_v_xij(bc, 0 /*tmp_res*/);
        if (tmp_fields > 5) ff_put_v_xij(bc, j);
        if (tmp_fields > 6) ff_put_v_xij(bc, tmp_match);
        if (tmp_fields > 7) ff_put_v_xij(bc, tmp_head_idx);
    }
    ff_put_v_xij(bc, nut->header_count - 1);
    for (i = 1; i < nut->header_count; i++) {
        ff_put_v_xij(bc, nut->header_len[i]);
        avio_write_xij(bc, nut->header[i], nut->header_len[i]);
    }
    // flags had been effectively introduced in version 4
    if (nut->version > 3)
        ff_put_v_xij(bc, nut->flags);
}
                              ...
                              AVStream *st, int i)
{
    ...
    case AVMEDIA_TYPE_VIDEO:    ff_put_v_xij(bc, 0); break;
    case AVMEDIA_TYPE_AUDIO:    ff_put_v_xij(bc, 1); break;
    case AVMEDIA_TYPE_SUBTITLE: ff_put_v_xij(bc, 2); break;
    default:                    ff_put_v_xij(bc, 3); break;
    }
    ff_put_v_xij(bc, 4);

    if (par->codec_tag) {
        avio_wl32_xij(bc, par->codec_tag);
    } else {
        av_log(avctx, AV_LOG_ERROR, "No codec tag defined for stream %d\n", i);
        return AVERROR(EINVAL);
    }

    ff_put_v_xij(bc, nut->stream[i].time_base - nut->time_base);
    ff_put_v_xij(bc, nut->stream[i].msb_pts_shift);
    ff_put_v_xij(bc, nut->stream[i].max_pts_distance);
    ff_put_v_xij(bc, par->video_delay);
    avio_w8_xij(bc, 0); /* flags: 0x1 - fixed_fps, 0x2 - index_present */

    ff_put_v_xij(bc, par->extradata_size);
    avio_write_xij(bc, par->extradata, par->extradata_size);

    switch (par->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
        ff_put_v_xij(bc, par->sample_rate);
        ff_put_v_xij(bc, 1);
        ff_put_v_xij(bc, par->channels);
        break;
    case AVMEDIA_TYPE_VIDEO:
        ff_put_v_xij(bc, par->width);
        ff_put_v_xij(bc, par->height);
        ...
        if (st->sample_aspect_ratio.num <= 0 ||
            ...
            ff_put_v_xij(bc, st->sample_aspect_ratio.num);
            ff_put_v_xij(bc, st->sample_aspect_ratio.den);
        }
        ff_put_v_xij(bc, 0); /* csp type -- unknown */
        break;
    default:
        break;
    }
...
static int write_globalinfo(NUTContext *nut, AVIOContext *bc)
{
    ...
    int ret          = avio_open_dyn_buf_xij(&dyn_bc);
    if (ret < 0)
        return ret;
    ...
    ff_put_v_xij(bc, 0); //stream_if_plus1
    ff_put_v_xij(bc, 0); //chapter_id
    ff_put_v_xij(bc, 0); //timestamp_start
    ff_put_v_xij(bc, 0); //length

    ff_put_v_xij(bc, count);

    dyn_size = avio_close_dyn_buf_xij(dyn_bc, &dyn_buf);
    avio_write_xij(bc, dyn_buf, dyn_size);
    av_free(dyn_buf);
    return 0;
}
...
static int write_streaminfo(NUTContext *nut, AVIOContext *bc, int stream_id) {
    ...
    if (count) {
        ff_put_v_xij(bc, stream_id + 1); //stream_id_plus1
        ff_put_v_xij(bc, 0); //chapter_id
        ff_put_v_xij(bc, 0); //timestamp_start
        ff_put_v_xij(bc, 0); //length
...
static int write_chapter(NUTContext *nut, AVIOContext *bc, int id)
{
    ...
    ret = avio_open_dyn_buf_xij(&dyn_bc);
    if (ret < 0)
        return ret;

    ff_put_v_xij(bc, 0);                                        // stream_id_plus1
    put_s(bc, id + 1);                                      // chapter_id
    put_tt(nut, nut->chapter[id].time_base, bc, ch->start); // chapter_start
    ff_put_v_xij(bc, ch->end - ch->start);                      // chapter_len
...
static int write_index(NUTContext *nut, AVIOContext *bc) {
    ...
    avio_wb64_xij(bc, 8 + payload_size + av_log2(payload_size) / 7 + 1 + 4*(payload_size > 4096));
...
static int nut_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && st->codecpar->sample_rate) {
            ...
            time_base = ff_choose_timebase_xij(s, st, 48000);
        }
    ...
    avio_write_xij(bc, ID_STRING, strlen(ID_STRING));
    avio_w8_xij(bc, 0);
    ...
    for (i = 0; i<pkt->side_data_elems; i++) {
        ...
        if (is_meta) {
            ...
            default:
                ...
                put_s(dyn_bc, -2);
                put_str(dyn_bc, "bin");
                ff_put_v_xij(dyn_bc, pkt->side_data[i].size);
                avio_write_xij(dyn_bc, data, pkt->side_data[i].size);
                sm_data_count++;
                break;
            case AV_PKT_DATA_PARAM_CHANGE:
                ...
                if (flags & AV_SIDE_DATA_PARAM_CHANGE_CHANNEL_LAYOUT) {
                    ...
                    put_str(dyn_bc, "u64");
                    ff_put_v_xij(bc, 8);
                    avio_write_xij(dyn_bc, data, 8); data+=8;
                    sm_data_count++;
                }
...
static int nut_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt->side_data_elems && nut->version > 3) {
        ret = avio_open_dyn_buf_xij(&sm_bc);
        if (ret < 0)
            return ret;
        ret = write_sm_data(s, sm_bc, pkt, 0);
        if (ret >= 0)
            ret = write_sm_data(s, sm_bc, pkt, 1);
        sm_size = avio_close_dyn_buf_xij(sm_bc, &sm_buf);
        if (ret < 0)
            goto fail;
        data_size += sm_size;
    }
...
//FIXME: Ensure store_sp is 1 in the first place.
    ...
    if (store_sp &&
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            int64_t dts_tb = av_rescale_rnd(pkt->dts,
                ...
                AV_ROUND_DOWN);
            int index = av_index_search_timestamp_xij(st, dts_tb,
        ...
        ff_put_v_xij(dyn_bc, sp_pos != INT64_MAX ? (nut->last_syncpoint_pos - sp_pos) >> 4 : 0);
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (flags & FLAG_STREAM_ID)
            length += ff_get_v_length_xij(pkt->stream_index);

        if (data_size % fc->size_mul != fc->size_lsb)
            continue;
        if (flags & FLAG_SIZE_MSB)
            length += ff_get_v_length_xij(data_size / fc->size_mul);

        if (flags & FLAG_CHECKSUM)
            length += 4;

        if (flags & FLAG_CODED_PTS)
            length += ff_get_v_length_xij(coded_pts);
    ...
    ffio_init_checksum_xij(bc, ff_crc04C11DB7_update_xij, 0);
    avio_w8_xij(bc, frame_code);
    if (flags & FLAG_CODED) {
        ff_put_v_xij(bc, (flags ^ needed_flags) & ~(FLAG_CODED));
        flags = needed_flags;
    }
    if (flags & FLAG_STREAM_ID)  ff_put_v_xij(bc, pkt->stream_index);
    if (flags & FLAG_CODED_PTS)  ff_put_v_xij(bc, coded_pts);
    if (flags & FLAG_SIZE_MSB )  ff_put_v_xij(bc, data_size / fc->size_mul);
    if (flags & FLAG_HEADER_IDX) ff_put_v_xij(bc, header_idx = best_header_idx);

    if (flags & FLAG_CHECKSUM)   avio_wl32_xij(bc, ffio_get_checksum_xij(bc));
    else                         ffio_get_checksum_xij(bc);

    if (flags & FLAG_SM_DATA) {
        avio_write_xij(bc, sm_buf, sm_size);
    }
    avio_write_xij(bc, pkt->data + nut->header_len[header_idx], pkt->size - nut->header_len[header_idx]);
