...
static void mpc8_parse_seektable(AVFormatContext *s, int64_t off)
{
    ...
    if(!(buf = av_malloc(size + AV_INPUT_BUFFER_PADDING_SIZE)))
        return;
    ret = avio_read_xij(s->pb, buf, size);
    if (ret != size) {
    ...
    for(i = 0; i < 2; i++){
        ...
        ppos[1 - i] = pos;
        av_add_index_entry_xij(s->streams[0], pos, i, 0, 0, AVINDEX_KEYFRAME);
    }
    for(; i < size; i++){
        ...
        av_add_index_entry_xij(s->streams[0], pos, i << seekd, 0, 0, AVINDEX_KEYFRAME);
        ppos[1] = ppos[0];
        ppos[0] = pos;
    }
    av_free(buf);
...
static int mpc8_read_header(AVFormatContext *s)
{
    ...
    pos = avio_tell(pb);
    avio_skip_xij(pb, 4); //CRC
    c->ver = avio_r8_xij(pb);
    if(c->ver != 8){
        avpriv_report_missing_feature(s, "Stream version %d", c->ver);
        return AVERROR_PATCHWELCOME;
    }
    c->samples = ffio_read_varlen_xij(pb);
    ffio_read_varlen_xij(pb); //silence samples at the beginning
    ...
    if (ff_get_extradata_xij(s, st->codecpar, pb, 2) < 0)
        return AVERROR(ENOMEM);
...
static int mpc8_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    while(!avio_feof_xij(s->pb)){
        ...
        if(tag == TAG_AUDIOPACKET){
            if(av_get_packet_xij(s->pb, pkt, size) < 0)
                return AVERROR(ENOMEM);
            pkt->stream_index = 0;
            pkt->duration     = 1;
            return 0;
