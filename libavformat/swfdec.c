...
retry:
    if (!z->avail_in) {
        int n = avio_read_xij(s->pb, swf->zbuf_in, ZBUF_SIZE);
        if (n < 0)
            return n;
...
static int swf_read_header(AVFormatContext *s)
{
    ...
    tag = avio_rb32_xij(pb) & 0xffffff00;
    avio_rl32_xij(pb);
    ...
    if (tag == MKBETAG('C', 'W', 'S', 0)) {
        ...
        swf->zpb = avio_alloc_context_xij(swf->zbuf_out, ZBUF_SIZE, 0, s,
        ...
        return AVERROR(EIO);
    /* skip rectangle size */
    nbits = avio_r8_xij(pb) >> 3;
    len = (4 * nbits - 3 + 7) / 8;
    avio_skip_xij(pb, len);
    swf->frame_rate = avio_rl16_xij(pb); /* 8.8 fixed */
    avio_rl16_xij(pb); /* frame count */
...
static AVStream *create_new_audio_stream(AVFormatContext *s, int id, int info)
{
    ...
    ast->codecpar->codec_id   = ff_codec_get_id_xij(swf_audio_codec_tags, info>>4 & 15);
...
static int swf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;) {
        ...
        if (tag == TAG_VIDEOSTREAM) {
            ...
            vst->codecpar->codec_id = ff_codec_get_id_xij(ff_swf_codec_tags, avio_r8_xij(pb));
            ...
            avio_r8_xij(pb);
            v = avio_r8_xij(pb);
            swf->samples_per_frame = avio_rl16_xij(pb);
            ...
            if (!ast)
                return AVERROR(ENOMEM);
            ast->duration = avio_rl32_xij(pb); // number of samples
            if (((v>>4) & 15) == 2) { // MP3 sound data record
                ast->skip_samples = avio_rl16_xij(pb);
                len -= 2;
            }
            len -= 7;
            if ((res = av_get_packet_xij(pb, pkt, len)) < 0)
                return res;
            pkt->pos = pos;
            ...
            const int ch_id   = avio_rl16_xij(pb);
            const int bmp_fmt = avio_r8_xij(pb);
            const int width   = avio_rl16_xij(pb);
            const int height  = avio_rl16_xij(pb);
            int pix_fmt;
            ...
            if (!st->codecpar->width && !st->codecpar->height) {
                ...
                ff_add_param_change_xij(pkt, 0, 0, 0, width, height);
            }
            pkt->pos = pos;
            pkt->stream_index = st->index;
...
bitmap_end_skip:
            ...
            avio_rl16_xij(pb); /* BITMAP_ID */
            len -= 2;
            if (len < 4)
                goto skip;
            if ((res = av_new_packet_ijk(pkt, len)) < 0)
                return res;
            if (avio_read_xij(pb, pkt->data, 4) != 4) {
                av_packet_unref_ijk(pkt);
                return AVERROR_INVALIDDATA;
            }
            if (AV_RB32(pkt->data) == 0xffd8ffd9 ||
                ...
                res = avio_read_xij(pb, pkt->data, pkt->size);
            } else {
                res = avio_read_xij(pb, pkt->data + 4, pkt->size - 4);
                if (res >= 0)
                    res += 4;
            }
            if (res != pkt->size) {
