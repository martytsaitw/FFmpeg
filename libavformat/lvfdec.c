...
static int lvf_read_header(AVFormatContext *s)
{
    ...
    avio_skip_xij(s->pb, 1012);

    while (!avio_feof_xij(s->pb)) {
        id          = avio_rl32_xij(s->pb);
        ...
        case MKTAG('0', '0', 'f', 'm'):
            ...
            avio_skip_xij(s->pb, 4);
            st->codecpar->codec_tag  = avio_rl32_xij(s->pb);
        ...
        case MKTAG('0', '1', 'f', 'm'):
            ...
            st->codecpar->codec_tag   = avio_rl16_xij(s->pb);
            st->codecpar->channels    = avio_rl16_xij(s->pb);
            st->codecpar->sample_rate = avio_rl16_xij(s->pb);
            avio_skip_xij(s->pb, 8);
            st->codecpar->bits_per_coded_sample = avio_r8_xij(s->pb);
            st->codecpar->codec_id    = ff_codec_get_id_xij(ff_codec_wav_tags,
                                                        st->codecpar->codec_tag);
            avpriv_set_pts_info_ijk(st, 32, 1, 1000);
            break;
        case 0:
            avio_seek_xij(s->pb, 2048 + 8, SEEK_SET);
            return 0;
        default:
        ...
        avio_seek_xij(s->pb, next_offset, SEEK_SET);
    }
