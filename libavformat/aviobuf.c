...
const AVClass ff_avio_class_xij = {
...
AVIOContext *avio_alloc_context_xij(
                  unsigned char *buffer,
                  int buffer_size,
                  int write_flag,
                  void *opaque,
                  ...
                  int64_t (*seek)(void *opaque, int64_t offset, int whence))
{
    ...
    if (!s)
        return NULL;
    ffio_init_context_xij(s, buffer, buffer_size, write_flag, opaque,
                  read_packet, write_packet, seek);
    return s;
}

void avio_context_free_xij(AVIOContext **ps)
{
    av_freep(ps);
}
...
void avio_w8_xij(AVIOContext *s, int b)
{
...
void avio_write_xij(AVIOContext *s, const unsigned char *buf, int size)
{
...
void avio_flush_xij(AVIOContext *s)
{
    int seekback = s->write_flag ? FFMIN(0, s->buf_ptr - s->buf_ptr_max) : 0;
    flush_buffer(s);
    if (seekback)
        avio_seek_xij(s, seekback, SEEK_CUR);
}

int64_t avio_seek_xij(AVIOContext *s, int64_t offset, int whence)
{
    int64_t offset1;
    int64_t pos;
    ...
    if ((!s->direct || !s->seek) &&
        ...
        s->eof_reached = 0;
        fill_buffer(s);
        return avio_seek_xij(s, offset, SEEK_SET | force);
    } else {
        int64_t res;
        if (s->write_flag) {
            flush_buffer(s);
...
int64_t avio_skip_xij(AVIOContext *s, int64_t offset)
{
    return avio_seek_xij(s, offset, SEEK_CUR);
}

int64_t avio_size_xij(AVIOContext *s)
{
    int64_t size;
...
int avio_feof_xij(AVIOContext *s)
{
    if(!s)
        return 0;
...
void avio_wl32_xij(AVIOContext *s, unsigned int val)
{
    avio_w8_xij(s, (uint8_t) val       );
    avio_w8_xij(s, (uint8_t)(val >> 8 ));
    avio_w8_xij(s, (uint8_t)(val >> 16));
    avio_w8_xij(s,           val >> 24 );
}

void avio_wb32_xij(AVIOContext *s, unsigned int val)
{
    avio_w8_xij(s,           val >> 24 );
    avio_w8_xij(s, (uint8_t)(val >> 16));
    avio_w8_xij(s, (uint8_t)(val >> 8 ));
...
int avio_put_str_xij(AVIOContext *s, const char *str)
{
    int len = 1;
    if (str) {
        len += strlen(str);
        avio_write_xij(s, (const unsigned char *) str, len);
    } else
        avio_w8_xij(s, 0);
    return len;
}
...
static inline int put_str16(AVIOContext *s, const char *str, const int be)
{
    ...
    while (*q) {
        ...
        PUT_UTF16(ch, tmp, be ? avio_wb16_xij(s, tmp) : avio_wl16_xij(s, tmp);
                  ret += 2;)
        continue;
invalid:
...
#define PUT_STR16(type, big_endian, posfix)                          \
int avio_put_str16 ## type ## posfix(AVIOContext *s, const char *str)  \
{                                                            \
...
{
    int i = ff_get_v_length_xij(val);

    while (--i > 0)
        avio_w8_xij(bc, 128 | (uint8_t)(val >> (7*i)));

    avio_w8_xij(bc, val & 127);
}

void avio_wl64_xij(AVIOContext *s, uint64_t val)
{
    avio_wl32_xij(s, (uint32_t)(val & 0xffffffff));
    avio_wl32_xij(s, (uint32_t)(val >> 32));
}

void avio_wb64_xij(AVIOContext *s, uint64_t val)
{
    avio_wb32_xij(s, (uint32_t)(val >> 32));
    avio_wb32_xij(s, (uint32_t)(val & 0xffffffff));
}

void avio_wl16_xij(AVIOContext *s, unsigned int val)
{
    avio_w8_xij(s, (uint8_t)val);
    avio_w8_xij(s, (int)val >> 8);
}

void avio_wb16_xij(AVIOContext *s, unsigned int val)
{
    avio_w8_xij(s, (int)val >> 8);
    avio_w8_xij(s, (uint8_t)val);
}

void avio_wl24_xij(AVIOContext *s, unsigned int val)
{
    avio_wl16_xij(s, val & 0xffff);
    avio_w8_xij(s, (int)val >> 16);
}

void avio_wb24_xij(AVIOContext *s, unsigned int val)
{
    avio_wb16_xij(s, (int)val >> 8);
    avio_w8_xij(s, (uint8_t)val);
}

void avio_write_marker_xij(AVIOContext *s, int64_t time, enum AVIODataMarkerType type)
{
...
static void fill_buffer(AVIOContext *s)
{
    ...
    if (s->read_packet && s->orig_buffer_size && s->buffer_size > s->orig_buffer_size) {
        if (dst == s->buffer && s->buf_ptr != dst) {
            int ret = ffio_set_buf_size_xij(s, s->orig_buffer_size);
            if (ret < 0)
...
int avio_read_xij(AVIOContext *s, unsigned char *buf, int size)
{
    int len, size1;
    ...
    if (size1 == size) {
        ...
        if (avio_feof_xij(s))  return AVERROR_EOF;
    }
    return size1 - size;
}
...
{
    int ret = avio_read_xij(s, buf, size);
    if (ret != size)
...
int ffio_read_indirect_xij(AVIOContext *s, unsigned char *buf, int size, const unsigned char **data)
{
    if (s->buf_end - s->buf_ptr >= size && !s->write_flag) {
        ...
        *data = buf;
        return avio_read_xij(s, buf, size);
    }
}

int avio_read_partial_xij(AVIOContext *s, unsigned char *buf, int size)
{
    int len;
...
unsigned int avio_rl16_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_r8_xij(s);
    val |= avio_r8_xij(s) << 8;
...
unsigned int avio_rl24_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_rl16_xij(s);
    val |= avio_r8_xij(s) << 16;
    return val;
}

unsigned int avio_rl32_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_rl16_xij(s);
    val |= avio_rl16_xij(s) << 16;
    return val;
}

uint64_t avio_rl64_xij(AVIOContext *s)
{
    uint64_t val;
    val = (uint64_t)avio_rl32_xij(s);
    val |= (uint64_t)avio_rl32_xij(s) << 32;
    return val;
}

unsigned int avio_rb16_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_r8_xij(s) << 8;
    val |= avio_r8_xij(s);
...
unsigned int avio_rb24_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_rb16_xij(s) << 8;
    val |= avio_r8_xij(s);
    return val;
}
unsigned int avio_rb32_xij(AVIOContext *s)
{
    unsigned int val;
    val = avio_rb16_xij(s) << 16;
    val |= avio_rb16_xij(s);
    return val;
}
...
{
    int len = ff_get_line_xij(s, buf, maxlen);
...
int64_t ff_read_line_to_bprint_overwrite_xij(AVIOContext *s, AVBPrint *bp)
{
    ...
    av_bprint_clear(bp);
    ret = ff_read_line_to_bprint_xij(s, bp);
    if (ret < 0)
        return ret;
...
int avio_get_str_xij(AVIOContext *s, int maxlen, char *buf, int buflen)
{
    int i;
...
#define GET_STR16(type, read, postfix) \
    int avio_get_str16 ##type ##postfix(AVIOContext *pb, int maxlen, char *buf, int buflen)\
{\
    char* q = buf;\
    int ret = 0;\
    if (buflen <= 0) \
...
GET_STR16(le, avio_rl16_xij, _xij)
GET_STR16(be, avio_rb16_xij, _xij)

#undef GET_STR16

uint64_t avio_rb64_xij(AVIOContext *s)
{
    uint64_t val;
    val = (uint64_t)avio_rb32_xij(s) << 32;
    val |= (uint64_t)avio_rb32_xij(s);
    return val;
}
...
int ffio_fdopen_xij(AVIOContext **s, URLContext *h)
{
    ...
    *s = avio_alloc_context_xij(buffer, buffer_size, h->flags & AVIO_FLAG_WRITE,
...
int ffio_rewind_with_probe_data_xij(AVIOContext *s, unsigned char **bufp, int buf_size)
{
...
int avio_open_xij(AVIOContext **s, const char *filename, int flags)
{
    return avio_open2_xij(s, filename, flags, NULL, NULL);
}

int ffio_open_whitelist_xij(AVIOContext **s, const char *filename, int flags,
...
int avio_open2_xij(AVIOContext **s, const char *filename, int flags,
               const AVIOInterruptCB *int_cb, AVDictionary **options)
{
    return ffio_open_whitelist_xij(s, filename, flags, int_cb, options, NULL, NULL);
}

int ffio_open2_wrapper(struct AVFormatContext *s, AVIOContext **pb, const char *url, int flags,
                       const AVIOInterruptCB *int_cb, AVDictionary **options)
{
    return ffio_open_whitelist_xij(pb, url, flags, int_cb, options, s->protocol_whitelist, s->protocol_blacklist);
}

int avio_close_xij(AVIOContext *s)
{
    AVIOInternal *internal;
    URLContext *h;
    ...
    avio_context_free_xij(&s);

    return ffurl_close(h);
}

int avio_closep_xij(AVIOContext **s)
{
    int ret = avio_close_xij(*s);
    *s = NULL;
    return ret;
}

int avio_printf_xij(AVIOContext *s, const char *fmt, ...)
{
    va_list ap;
    ...
    va_end(ap);
    avio_write_xij(s, buf, strlen(buf));
    return ret;
}

int avio_pause_xij(AVIOContext *s, int pause)
{
    if (!s->read_pause)
        return AVERROR(ENOSYS);
...
int64_t avio_seek_time_xij(AVIOContext *s, int stream_index,
...
int avio_read_to_bprint_xij(AVIOContext *h, AVBPrint *pb, size_t max_size)
{
    int ret;
    char buf[1024];
    while (max_size) {
        ret = avio_read_xij(h, buf, FFMIN(max_size, sizeof(buf)));
        if (ret == AVERROR_EOF)
            return 0;
        if (ret <= 0)
            return ret;
...
int avio_accept_xij(AVIOContext *s, AVIOContext **c)
{
    int ret;
    AVIOInternal *internal = s->opaque;
    ...
    if (ret < 0)
        return ret;
    return ffio_fdopen_xij(c, cc);
}

int avio_handshake_xij(AVIOContext *c)
{
...
static int url_open_dyn_buf_internal(AVIOContext **s, int max_packet_size)
{
    ...
    *s = avio_alloc_context_xij(d->io_buffer, d->io_buffer_size, 1, d, NULL,
...
int avio_open_dyn_buf_xij(AVIOContext **s)
{
...
int avio_get_dyn_buf_xij(AVIOContext *s, uint8_t **pbuffer)
{
    DynBuffer *d;
...
int avio_close_dyn_buf_xij(AVIOContext *s, uint8_t **pbuffer)
{
    DynBuffer *d;
    int size;
    ...
    if (!s->max_packet_size) {
        avio_write_xij(s, padbuf, sizeof(padbuf));
