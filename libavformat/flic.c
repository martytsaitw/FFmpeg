...
static int flic_read_header(AVFormatContext *s)
{
    ...
    if (AV_RL16(&preamble[4]) == FLIC_TFTD_CHUNK_AUDIO) {
        ...
        avpriv_set_pts_info_ijk(st, 64, ast->codecpar->block_align, FLIC_TFTD_SAMPLE_RATE);
        avpriv_set_pts_info_ijk(ast, 64, 1, FLIC_TFTD_SAMPLE_RATE);
        ...
        avpriv_set_pts_info_ijk(st, 64, speed, 70);
    } else if ((magic_number == FLIC_FILE_MAGIC_2) ||
               (magic_number == FLIC_FILE_MAGIC_3)) {
        avpriv_set_pts_info_ijk(st, 64, speed, 1000);
    } else {
