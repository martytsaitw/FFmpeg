...
start:
    ...
    for (;;) {
        ...
        *q = '\0';
        ...
        if (line_count == 0) {
            ...
            ff_rtsp_parse_line_ijk(s, reply, p, rt, method);
    ...
    if (request) {
        ...
        /* If method is set, this is called from ff_rtsp_send_cmd_ijk,
 ...
 */
static int rtsp_send_cmd_with_content_async_ijk(AVFormatContext *s,
...
int ff_rtsp_send_cmd_ijk_async_ijk(AVFormatContext *s, const char *method,
                           const char *url, const char *headers)
{
    return rtsp_send_cmd_with_content_async_ijk(s, method, url, headers, NULL, 0);
}

int ff_rtsp_send_cmd_ijk(AVFormatContext *s, const char *method, const char *url,
                     const char *headers, RTSPMessageHeader *reply,
                     unsigned char **content_ptr)
{
    return ff_rtsp_send_cmd_ijk_with_content(s, method, url, headers, reply,
                                         content_ptr, NULL, 0);
}

int ff_rtsp_send_cmd_ijk_with_content(AVFormatContext *s,
                                  const char *method, const char *url,
                                  const char *header,
...
retry:
    ...
    if ((ret = rtsp_send_cmd_with_content_async_ijk(s, method, url, header,
                                                send_content,
                                                send_content_length)))
        return ret;

    if ((ret = ff_rtsp_read_reply_ijk(s, reply, content_ptr, 0, method) ) < 0)
        return ret;
    attempts++;
                              ...
                              int lower_transport, const char *real_challenge)
{
    ...
    for (j = rt->rtp_port_min + port_off, i = 0; i < rt->nb_rtsp_streams; ++i) {
        ...
        ff_rtsp_send_cmd_ijk(s, "SETUP", rtsp_st->control_url, cmd, reply, NULL);
        ...
        if ((err = ff_rtsp_open_transport_ctx_ijk(s, rtsp_st)))
            goto fail;
    }
...
redirect:
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (rt->server_type == RTSP_SERVER_REAL)
            av_strlcat(cmd,
                       ...
                       sizeof(cmd));
        ff_rtsp_send_cmd_ijk(s, "OPTIONS", rt->control_uri, cmd, reply, NULL);
    ...
    do {
        ...
        err = ff_rtsp_make_setup_request_ijk(s, host, port, lower_transport,
                                 rt->server_type == RTSP_SERVER_REAL ?
                                     real_challenge : NULL);
        if (err < 0)
            goto fail;
...
static int parse_rtsp_message(AVFormatContext *s)
{
    ...
    if (rt->rtsp_flags & RTSP_FLAG_LISTEN) {
        ...
        RTSPMessageHeader reply;
        ret = ff_rtsp_read_reply_ijk(s, &reply, NULL, 0, NULL);
        if (ret < 0)
            return ret;
        /* XXX: parse message */
        if (rt->state != RTSP_STATE_STREAMING)
...
static int udp_read_packet_ijk(AVFormatContext *s, RTSPStream **prtsp_st,
...
static int pick_stream_ijk(AVFormatContext *s, RTSPStream **rtsp_st,
                       ...
                       int64_t wait_end)
{
    ...
    case RTSP_LOWER_TRANSPORT_UDP:
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
        len = udp_read_packet_ijk(s, rtsp_st, rt->recvbuf, RECVBUF_SIZE, wait_end);
    ...
    case RTSP_LOWER_TRANSPORT_CUSTOM:
        ...
        len = pick_stream_ijk(s, rtsp_st, rt->recvbuf, len);
        if (len > 0 && (*rtsp_st)->transport_priv && rt->transport == RTSP_TRANSPORT_RTP)
...
static int sdp_read_header(AVFormatContext *s)
{
    ...
    err = ff_sdp_parse_ijk(s, content);
    av_freep(&content);
    if (err) goto fail;
...
AVInputFormat ff_sdp_demuxer = {
    ...
    .read_packet    = ff_rtsp_fetch_packet_ijk,
